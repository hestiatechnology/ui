...
class ToastDemoComponent {
  ...
  info() {
    this.toast.info('Information', 'Lot LOT-2024-0042 has been updated.');
  }
  success() {
    this.toast.success('Certified!', 'GOTS certification approved.');
  }
  warning() {
    this.toast.warning('Idle warning', 'Machine CK-01 has been idle for 2 hours.');
  }
  error() {
    this.toast.error('Quality check failed', 'Batch quarantined for review.');
  }
}
